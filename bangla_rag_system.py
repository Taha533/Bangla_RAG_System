...
# # Configuration
# PDF_PATH = "/media/nsl53/hdd1/Taha/project_work/RAG_Application/paper.pdf" #"F:\\RAG_based_App\\doctors_list.pdf"
# CHUNK_SIZE = 1000
# CHUNK_OVERLAP = 200
...
# # Evaluation
# # def evaluate_rag(query: str, expected: str, rag_chain: RetrievalQA):
# #     try:
# #         answer = rag_chain.run(query)
# #         embeddings = test_embeddings()
# #         sim = np.dot(embeddings.embed_query(answer), embeddings.embed_query(expected)) / (
# #             np.linalg.norm(embeddings.embed_query(answer)) * np.linalg.norm(embeddings.embed_query(expected))
# #         )
# #         return {
# #             "query": query,
# #             "expected": expected,
...
#             print(f"[Doc {i+1}]: {doc.page_content[:500]}")  # Show first 500 chars
#             print("-" * 50)

#         # Run the chain (use .invoke if you're on LangChain >= 0.1.0)
#         # answer = rag_chain.run(query)
#         answer = rag_chain.invoke({"query": query})  # Recommended
...
# def main():
#     try:
#         docs = load_and_preprocess_pdf(PDF_PATH)
#         vector_store = create_vector_store(docs)
#         rag_chain = create_rag_chain(vector_store)
 
#         # sample_queries = [
#         #     {"query": "Who are the cardiologists listed in the document?", "expected": "Dr. John Doe, Dr. Jane Smith"},
#         #     {"query": "What is the contact number of Dr. John Doe?", "expected": "123-456-7890"},
#         #     {"query": "Which doctors specialize in neurology?", "expected": "Dr. Alice Brown"}
#         # ]
...
#         {"query": "বিয়ের সময় কল্যাণীর প্রকৃ ত বয়স কত ছিল?", "expected": "১৫ বছর"}
#     ]
...
# # Configuration
# PDF_PATH = "/media/nsl53/hdd1/Taha/project_work/RAG_Application/paper.pdf"
# CHUNK_SIZE = 1000
# CHUNK_OVERLAP = 200
...
#         for doc in data:
#             # Replace multiple spaces and normalize Unicode for Bengali text
#             doc.page_content = re.sub(r'\s+', ' ', doc.page_content).strip()
#             # Normalize Bengali Unicode (optional, depending on font issues)
#             doc.page_content = doc.page_content.encode().decode('utf-8', errors='replace')

#         # Use a token-based splitter to preserve Bengali word boundaries
#         text_splitter = RecursiveCharacterTextSplitter(
#             chunk_size=CHUNK_SIZE,
#             chunk_overlap=CHUNK_OVERLAP,
#             length_function=len,
#             separators=["\n\n", "\n", "।", " ", ""],  # Include Bengali punctuation
#         )
#         return text_splitter.split_documents(data)
#     except Exception as e:
#         logger.error(f"Error loading PDF: {e}")
...
# # HuggingFace embeddings optimized for Bengali
...
#         llm = initialize_llm()
#         prompt_template = """
#         Use the following pieces of context to answer the question in Bengali. If you don't know the answer, say "উত্তর পাওয়া যায়নি" (Answer not found).
#         Context: {context}
#         Question: {question}
#         Answer:
#         """
...
#         logger.info("\n--- Retrieved Context ---")
#         for i, doc in enumerate(retrieved_docs):
#             logger.info(f"[Doc {i+1}]: {doc.page_content[:500]}")
#             logger.info("-" * 50)

#         # Run the chain
#         result = rag_chain.invoke({"query": query})
#         answer = result.get("result", "")  # Extract the 'result' from the dictionary
...
# # Load and preprocess PDF with bangla_pdf_ocr
# def load_and_preprocess_pdf(pdf_path: str):
#     try:
#         if not os.path.exists(pdf_path):
#             raise FileNotFoundError(f"PDF not found: {pdf_path}")

#         logger.info(f"Loading PDF with bangla_pdf_ocr: {pdf_path}")
